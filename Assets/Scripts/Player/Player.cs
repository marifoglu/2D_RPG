...
public class Player : Entity
{
    ...
    {
        // Temporary ignore after exiting a ladder to avoid immediate re-detect/re-grab
        ...
        if (ladderCollider != null)
        {
            ...
            {
                // Don't set ladder if player is above the ladder top (can't grab from above)
                Vector2 ladderTop = detectedLadder.GetTopPosition();
                if (transform.position.y <= ladderTop.y + 0.05f) // small tolerance
                {
        ...
        // If not in ladder zone anymore, clear
